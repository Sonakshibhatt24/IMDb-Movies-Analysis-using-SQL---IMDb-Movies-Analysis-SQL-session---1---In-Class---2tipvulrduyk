...
-- minimum_median_rating=1, maximum_median_ratings=100


-- 2. Identify the top 10 movies based on average rating.

with cte as (select title,avg_rating as average_rating from movie
left join ratings on movie_id=id),
cte2 as (select *, row_number() over (order by average_rating desc) as ranks from cte)
(select * from cte2 where ranks<=10);
