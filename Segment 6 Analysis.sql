...
select id,title, genre, duration, sum(duration) over (partition by genre order by duration) as running_total, 
avg(duration) over (partition by genre order by duration) as moving_average
...
with cte as (select * from (select year, genre,count(id) as movie_count, 
row_number() over (partition by year order by count(id) desc) as ranks from movie m
left join genre g on m.id=g.movie_id
group by genre,year) a where ranks<=3)
(select * from (select id, title, genre, year, worlwide_gross_income as worldwide_income,
...
-- In 2017, The highest grossing movie is Thank You for Your Service, followed by the Healer and so on.
-- In 2018, it is Antony & Cheopatra followed by La Fuitina sbagliata and so on
-- In 2019, it is Prescience, Followed by Joker and so on
